...
export function exportExcel(
  fileName = 'download',
  array,
  // [{sheetName = '', columns = [], data = [], header = null, merges = []}],
  callback = () => {},
) {
  let workbook = new Workbook()
  array.forEach(({sheetName='', columns=[], data=[], header=[], merges=[]}) => {
    // 处理数据格式
    ...
    const worksheetName = sheetName || 'Sheet'
      ...
      Object.keys(sheetList).forEach(key => {
        ...
        if (RANGE_KEY in worksheet) {
          ...
          Object.keys(worksheet).forEach(key => {
            ...
            // }
            if(worksheet[key].t === 'n' && isNaN(worksheet[key].w)){
